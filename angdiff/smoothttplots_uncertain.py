...
qparams=ts.theil_sen(wolq_flat,galq_flat)
uparams=ts.theil_sen(wolu_flat,galu_flat)
coeffq=qparams[0]
offsetq=qparams[1]
coeffu=uparams[0]
offsetu=uparams[1]
...
print 'with a 95% confidence interval bounded by {0} and {1}'.format(qparams[4],qparams[5])
print 'The GALFACTS U data contains a spurious offset of {0} K'.format(offsetu)
print 'with a 95% confidence interval bounded by {0} and {1}'.format(uparams[4],uparams[5])
