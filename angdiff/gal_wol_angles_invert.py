...
#####Here we are assuming that the maps are switched, i.e. galfacts q actually looks like wolleben u and galfacts u looks like -wol q
...
savetitle=field+'_TTplots_invert.pdf'

fig.savefig(savetitle,dpi=150)
...
mean_err_deg=np.mean(angle_err_deg)
...
difffileout=field+'_diff_map_invert.fits'
errfileout=field+'_diff_err_map_invert.fits'
galangfileout=field+'_galfacts_angles_invert.fits'
wolangfileout=field+'_wolleben_angles_invert.fits'

fits.writeto(difffileout,angle_diff_deg,header=diffmap_head)
fits.writeto(errfileout,angle_err,header=differr_head)
