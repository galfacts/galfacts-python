...
#Use to generate noise plot data for the cubes. arguments are I, Q, U ,V, W cubes in that order.
...
s=np.shape(i_err)
i_reduced=np.reshape(i_err,(s[0],s[1]*s[2]))
q_reduced=np.reshape(q_err,(s[0],s[1]*s[2]))
u_reduced=np.reshape(u_err,(s[0],s[1]*s[2]))
v_reduced=np.reshape(v_err,(s[0],s[1]*s[2]))
w_reduced=np.reshape(weights,(s[0],s[1]*s[2]))
...
scale_factor= 26.5/np.sqrt(0.2*4.2E5*2.) #this is given by the radiometer thermal noise equation for a given system temperature, bandwidth and integration time (26.5K, 420 kHz and 0.2s), with a 1/sqrt(2) factor reduction in the noise due to gaussian interpolation in the gridding.
