...
slope,offset,c,d,e=stats.linregress(np.log10(bins_axis[nonzero]),np.log10((ee_average[nonzero]+bb_average[nonzero])/2.))

print slope,offset

fig,ax=plt.subplots(figsize=(8,8))
title='APS of GALFACTS DR3.1 '+field+' chunk{0}'.format(chunk)
ax.set_title(title)
...
#),label='line of slope 2 with calculated offset')
    ...
    ax.plot(bins_axis[15:],10**(slope*np.log10(bins_axis[15:])+offset),'k--',linewidth=2,label='power-law fit')
...
if scale == 'dl':
    ...
    fig.savefig("./aps/"+field+"_aps_dqa3.1_c"+str(chunk)+"_cl_withps_talk.pdf",dpi=100)
plt.show()
