...
def chunk_area(field):
    ...
    omega,erromega=scipy.integrate.dblquad(dA_Sphere, rastart, rastop, lambda theta: decstart, lambda theta: decstop )
...
power_average[nonzero]=omega*power_hist[nonzero]/ell_hist[nonzero]
if scale == 'dl':
...
ee_average[nonzero_ee]=omega*ee_hist[nonzero_ee]/ell_hist[nonzero_ee]
if scale == 'dl':
...
bb_average[nonzero_bb]=omega*bb_hist[nonzero_bb]/ell_hist[nonzero_bb]
if scale =='dl':
...
title='TT,EE and BB power spectra of GALFACTS DR3.1 {1} map, chunk {0}'.format(chunk,field)
ax.set_title(title)
...
if scale == 'dl':
    fig.savefig(field+"_aps_dqa3.1_c"+str(chunk)+"_dl_omega.pdf",dpi=100)
else:
    fig.savefig(field+"_aps_dqa3.1_c"+str(chunk)+"_cl_omega.pdf",dpi=100)
plt.show()
